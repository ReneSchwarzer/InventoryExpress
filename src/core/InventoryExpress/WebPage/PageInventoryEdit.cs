...
namespace InventoryExpress.WebPage
{
    ...
    public sealed class PageInventoryEdit : PageWebApp, IPageInventory
    {
        ...
        private void OnInitializeFormular(object sender, FormularEventArgs e)
        {
            ...
            //var attributes = ViewModel.GetAttributes(Form.Template.Value);
            //var attributesForm = new List<ControlFormularItemInputTextBox>();

            //Form.Attributes.Items.Clear();

            //foreach (var attribute in attributes)
            //{
            //    Form.Attributes.Items.Add(new ControlFormularItemInputTextBox()
            //    {
        ...
        private void OnFillFormular(object sender, FormularEventArgs e)
        {
            var guid = e.Context.Request.GetParameter("InventoryID")?.Value;
            var inventory = ViewModel.GetInventory(guid);
            var attributes = ViewModel.GetAttributes(inventory.Template);
            //var attributesForm = new List<ControlFormularItemInputTextBox>();
            
            Form.InventoryName.Value = inventory?.Name;
            Form.Manufacturer.Value = inventory.Manufacturer?.ID;
            Form.Manufacturer.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.Manufacturer?.ID, Label = inventory.Manufacturer?.Name });
            Form.Location.Value = inventory.Location?.ID;
            Form.Location.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.Location?.ID, Label = inventory.Location?.Name });
            Form.Supplier.Value = inventory.Supplier?.ID;
            Form.Supplier.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.Supplier?.ID, Label = inventory.Supplier?.Name });
            Form.LedgerAccount.Value = inventory.LedgerAccount?.ID;
            Form.LedgerAccount.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.LedgerAccount?.ID, Label = inventory.LedgerAccount?.Name });
            Form.CostCenter.Value = inventory.CostCenter?.ID;
            Form.CostCenter.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.CostCenter?.ID, Label = inventory.CostCenter?.Name });
            Form.Condition.Value = inventory.Condition?.ID;
            Form.Condition.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.Condition?.ID, Label = inventory.Condition?.Name });
            Form.Parent.Value = inventory.Parent?.ID;
            Form.Parent.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.Parent?.ID, Label = inventory.Parent?.Name });
            Form.Template.Value = inventory.Template?.ID;
            Form.Template.Options.Add(new ControlFormularItemInputSelectionItem() { ID = inventory.Template?.ID, Label = inventory.Template?.Name });

...
{
    /// <summary>
    /// Eine Seite zum Bearbeiten oder zur Neuanlage von Herstellern und deren Attribute
    /// </summary>
    public sealed partial class PageManufacturerItemEdit : Page
    {
        /// <summary>
        /// Konstruktor
        /// </summary>
        public PageManufacturerItemEdit()
        {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            else
            {
                ...
                Titel.Text = resourceLoader.GetString("PageManufacturerItemAddLabel/Text");

                DataContext = new Model.Manufacturer();
            }
        }

        /// <summary>
        /// Wird aufgerufen, wenn der Hersteller gespeichert werden soll
        /// </summary>
        ...
        private async void OnSaveAndNavigateBack(object sender, RoutedEventArgs e)
        {
            ...
            if (manufacturer != null)
            {
                ...
                else if (!Model.ViewModel.Instance.Manufacturers.Contains(manufacturer) &&
                          Model.ViewModel.Instance.Manufacturers.Find(f => f != null && !string.IsNullOrWhiteSpace(f.Name) && f.Equals(manufacturer.Name)) != null)
                {
                    MessageDialog msg = new MessageDialog
                    (
        ...
        /// <summary>
        /// Wird aufgerufen, wenn der Hersteller gelöscht werden soll
        /// </summary>
        /// <param name="sender">Der Auslöser des Events</param>
        /// <param name="e">Die Eventparameter</param>
        private async void OnDeleteAndNavigateBack(object sender, RoutedEventArgs e)
        {
            var resourceLoader = ResourceLoader.GetForCurrentView();
            var manufacturer = DataContext as Manufacturer;
            var exist = await ApplicationData.Current.RoamingFolder.TryGetItemAsync(manufacturer.ID + ".manufacturer");

            if (manufacturer != null && exist != null)
            {
                MessageDialog msg = new MessageDialog
                (
                ...
                {
                    // Daten löschen
                    Model.ViewModel.Instance.Manufacturers.Remove(manufacturer);

                    // Datei löschen
                    var file = await ApplicationData.Current.RoamingFolder.GetFileAsync(manufacturer.ID + ".manufacturer");
        ...
        private async void OnOpenImageAsync(object sender, RoutedEventArgs e)
        {
            ...
            if (file != null)
            {
                manufacturer.ImageBase64 = await Item.ConvertToIBase64Async(file, 200);
            }
        }

...
namespace InventoryExpress.Model
{
    public partial class ViewModel
    {
        ...
        public static void AddOrUpdateMedia(WebItemEntityMedia media)
        {
            var root = Path.Combine(ModuleContext.DataPath, "media");

            lock (DbContext)
            {
                var availableEntity = DbContext.Media.Where(x => x.Id == media.Id).FirstOrDefault();
                ...
                {
                    // update
                    availableEntity.Guid = media.Guid;
